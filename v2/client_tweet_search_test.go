...
func TestClient_TweetSearch(t *testing.T) {
	type fields struct {
		Authorizer Authorizer
		Client     *http.Client
		Host       string
	...
	type args struct {
		query string
		opts  TweetSearchOpts
	}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    *TweetSearchResponse
		wantErr bool
	}{
		{
			name: "success",
			fields: fields{
				...
				Client: mockHTTPClient(func(req *http.Request) *http.Response {
					...
					if strings.Contains(req.URL.String(), string(tweetSearchEndpoint)) == false {
						log.Panicf("the url is not correct %s %s", req.URL.String(), tweetSearchEndpoint)
					}
					body := `{
						"data": [
						  {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.TweetSearch(context.Background(), tt.args.query, tt.args.opts)
			if (err != nil) != tt.wantErr {
				t.Errorf("Client.TweetSearch() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Client.TweetSearch() = %v, want %v", got, tt.want)
			}
		})
	}
}
